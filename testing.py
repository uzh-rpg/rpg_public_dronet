...
"""
Evaluate a model on the test dataset (real gates)
"""
...
def compute_gate_localization_accuracy(predictions, ground_truth, topn=1):
    ...
    for i, pred in enumerate(predictions):
        ...
        for gt in np.argwhere(ground_truth[i]):
            if gt in indices:
                valid += 1
                break
...
def save_visual_output(input_img, prediction, ground_truth, index):
    ...
    gt_windows = np.argwhere(ground_truth)
